...
const NotebookIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg data-testid="geist-icon" viewBox="0 0 16 16" strokeLinejoin="round" {...props}>
    <path
      fillRule="evenodd"
      clipRule="evenodd"
...
const SunIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg data-testid="sun-icon" viewBox="0 0 16 16" {...props}>
    <path
      fillRule="evenodd"
      clipRule="evenodd"
...
export const themeLsStore = {
  setTheme(theme: "light" | "dark" | null) {
    if (theme === null) {
      localStorage.removeItem("force-theme");
    } else {
      localStorage.setItem("force-theme", theme);
    }
    eventTarget.dispatchEvent(new Event("theme-changed"));
  },
  subscribe(listener: () => void) {
    eventTarget.addEventListener("theme-changed", listener);
    return () => {
      eventTarget.removeEventListener("theme-changed", listener);
    };
  },
  getSnapshot() {
    return localStorage.getItem("force-theme") as "light" | "dark" | null;
  },
};

export default function ThemeSwitcher() {
  const lsValue = useSyncExternalStore(
    themeLsStore.subscribe,
    themeLsStore.getSnapshot,
  );

  return (
    <fieldset className={styles.wrapper}>
      <legend className={styles.legend}>Select a display theme:</legend>

      <span className={styles.span} style={{ height: "100%", position: "relative" }}>
        <input
          ...
          name="theme"
          value="system"
          onChange={() => themeLsStore.setTheme(null)}
          checked={lsValue === null}
        />
      ...
      <span className={styles.span} style={{ height: "100%", position: "relative" }}>
        <input
          ...
          name="theme"
          value="light"
          onChange={() => themeLsStore.setTheme("light")}
          checked={lsValue === "light"}
        />
      ...
      <span className={styles.span} style={{ height: "100%", position: "relative" }}>
        <input
          ...
          name="theme"
          value="dark"
          onChange={() => themeLsStore.setTheme("dark")}
          checked={lsValue === "dark"}
        />
